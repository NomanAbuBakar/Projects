...
<div class="container">
	...
	$sql = "SELECT * FROM friend WHERE (send_status = 1 AND recieve_status = 1) AND (user_name = '$curr_user' OR friend_name = '$curr_user')";
...
?>
	...
	{
		if($curr_user == $row['friend_name'] )
		{
			$friend_username = $row['user_name'];
		}
		else
		{
			$friend_username = $row['friend_name'];
		}
 ...
 ?>
					...
					<td><?= $row2['Lname']?></td>
					<td><?= $row2['Username']?></td>
					<td> <a href="un_friend.php?id=<?= $row2['id']?>&user=<?= $usrName?>">Unfriend</a> </td>
					<br><br>
					</tr>      
